...
class Solution:
    def findDifference(self, nums1: list[int], nums2: list[int]) -> list[list[int]]:
        """Return two lists of unique elements in num1 and num2."""

        # Use my own hashmap implementation
        hashmap1 = MyHashMap()
        hashmap2 = MyHashMap()

        # Insert the elements of nums1 into hashmap1
        for num in nums1:
            hashmap1[num] = num

        # Insert the elements of nums2 into hashmap2
        for num in nums2:
            hashmap2[num] = num

        # Create two lists to store the unique elements in nums1 and nums2
        unique1 = []
        unique2 = []

        # Iterate through the elements of nums1
        for num in nums1:
            # If the element is not in nums2, add it to unique1
            if num not in hashmap2 and num not in unique1:
                unique1.append(num)

        # Iterate through the elements of nums2
        for num in nums2:
            # If the element is not in nums1, add it to unique2
            if num not in hashmap1 and num not in unique2:
                unique2.append(num)
...
if __name__ == "__main__":
    nums1 = [-4, -4, -1, -3, -3, 6, -1, 8]
    nums2 = [8, -1, -1, 0, -6, -3, 7, 8]
    print(Solution().findDifference(nums1, nums2))  # [[-4, -3, 6], [0, -6, 7]]
